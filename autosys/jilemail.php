...
foreach($job_names as $job_name) {
  ...
  if ($job_prefix!==FALSE) {
     fwrite($jilfile_handle,
           "update_job:$job_name send_notification:F".
           ' notification_emailaddress:"'.$jobs[$job_prefix]['email'].'"'.
           ' notification_msg:"Job '.$job_name.' has failed in Production."'.
	   "\n");
  }
}
